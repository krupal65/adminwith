...
const ProductListingPage = ({ params }) => {
    ...
    useEffect(() => {
        ...
        getData(`/api/products?populate=*&[filters][categories][title]=${sessionStorage.getItem('cat')}`);
       // getData(`/api/products?populate=*&[filters][categories][title]=${sessionStorage.getItem('cat')}&[filters][sub-cats][id]=${params.subcatid}`);
    }, []);
    ...
    const filterByRating = (keyword) => {
        fetchDataFromApi(`/api/products?populate=*&[filters][categories][title]=${sessionStorage.getItem('cat')}`).then(res => {
    
            res.data.length!==0 && res.data.map((item)=>{
                item.attributes.sub_cats.data.length!==0 &&
                item.attributes.sub_cats.data.map((subCat)=>{
                    if(parseFloat(params.subcatid)===parseFloat(subCat.id)){
                        if(parseFloat(keyword) === parseFloat(item.attributes.rating)){
                            arr.push(item);
                        }
                ...
                <div className='container-fluid'>

                    {
                        ...
                        <ul className='list list-inline mb-0'>
                            ...
                            <li className='list-inline-item'>
                                <Link href={`/${sessionStorage.getItem('cat')}`} className='text-capitalize'>{sessionStorage.getItem('cat')} </Link>
                            </li>
                            <li className='list-inline-item'>
                                <Link href={`/subcat/${params.subcatid}`} className='text-capitalize'>{currentSubCat} </Link>
                            </li>
