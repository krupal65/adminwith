                    ...
                    <div className='row'>
                            ...
                            <div className='productZoom'>
                                <Slider {...settings2} className='zoomSliderBig' ref={zoomSliderBig}>

                                    {
                                        ...
                                        currentProduct?.attributes?.productImages?.data?.length !== 0 &&
                                        currentProduct?.attributes?.productImages?.data?.map((imgUrl, index) => {
                                            return (
                            ...
                            <Slider {...settings} className='zoomSlider' ref={zoomSlider}>

                                {
                                    ...
                                    currentProduct?.attributes?.productImages?.data?.map((imgUrl, index) => {
                                        return (
                                            <div className='item' key={index}>
                                                <img src={`http://localhost:1337${imgUrl.attributes.url}`} className='w-100' onClick={() => goto(index)} alt="productImage" />
                                            </div>
                                        )
                                    })
                                }
                        ...
                        <div className='col-md-7 productInfo'>
                            ...
                            <div className='d-flex align-items-center mb-4 mt-3'>
                                <Rating name="half-rating-read" value={parseFloat(currentProduct?.attributes?.rating)} precision={0.5} readOnly />
                                <span className='text-dark ml-2'>({reviewsArr.length} reviews)</span>
                            </div>


                            <div className='priceSec d-flex align-items-center mb-3'>
                                <span className='text-g priceLarge'>Rs {currentProduct?.attributes?.price}</span>
                                <div className='ml-3 d-flex flex-column'>
                                    <span className='text-org'>{currentProduct?.attributes?.discount}% Off</span>
                                    <span className='text-light oldPrice'>Rs {currentProduct?.attributes?.oldPrice}</span>
                                </div>
                            </div>

                            <p className="mb-5" style={{ fontSize: '25px' }}>{currentProduct?.attributes?.description}</p>



                            {
                                currentProduct?.attributes?.product_rams !== undefined && currentProduct?.attributes?.product_rams?.data?.length !== 0 &&
                                <>
                                    ...
                                    <div className='productSize d-flex align-items-center pt-0'>
                                        ...
                                        <ul className={`list list-inline mb-0 pl-4`}>
                                            {
                                                currentProduct?.attributes.product_rams !== undefined && currentProduct?.attributes.product_rams.data.map((prodRam, index) => {
                                                    return (
                                                        <li className='list-inline-item'><a className={`tag ${activeSize === index ? 'active' : ''}`} onClick={() => isActive(index)}>{prodRam.attributes.ram}</a></li>
                                                    )
                                                })
                                            }
                                        </ul>
                            ...
                            {
                                currentProduct?.attributes?.product_weights !== undefined && currentProduct?.attributes?.product_weights?.data.length !== 0 &&
                                <div className='productSize d-flex align-items-center'>
                                    <span>Size / Weight:</span>
                                    <ul className='list list-inline mb-0 pl-4'>
                                        {
                                            currentProduct?.attributes?.product_weights !== undefined && currentProduct?.attributes?.product_weights?.data?.map((prodWeight, index) => {
                                                return (
                                                    <li className='list-inline-item'><a className={`tag ${activeSize === index ? 'active' : ''}`} onClick={() => isActive(index)}>{prodWeight.attributes.weight}</a></li>
                                                )
                                            })
                                        }
                                    </ul>
                            ...
                            {/* {
                                currentProduct?.attributes?.fashion_product_sizes !== undefined && currentProduct?.attributes.fashion_product_sizes?.data?.length !== 0 &&
                                <div className='productSize d-flex align-items-center'>
                                    <span>Size :</span>
                                    <ul className='list list-inline mb-0 pl-4'>
                                        {
                                            currentProduct?.attributes?.fashion_product_sizes !== undefined && currentProduct?.attributes?.fashion_product_sizes1?.data?.map((prodSize, index) => {
                                                return (
                                                    <li className='list-inline-item'><a className={`tag ${activeSize === index ? 'active' : ''}`} onClick={() => isActive(index)}>{prodSize.attributes.size}</a></li>
                                                )
                                            })
                                        }
                                    </ul>
                            ...
                            <div className='d-flex align-items-center mt-5'>
                                ...
                                <div className='d-flex align-items-center'>
                                    ...
                                    {
                                        context.windowWidth > 992 && <Button className={`btn-g btn-lg addtocartbtn`} onClick={() => addToCart(currentProduct)}><ShoppingCartOutlinedIcon />
                                            {
                                                isAdded === true  ? 'Added' : 'Add To Cart'
                                            }
                                        </Button>
                                                    ...
                                                    return (
                                                        ...
                                                        <div className='card p-4 reviewsCard flex-row' key={index}>
                                                            <div className='image'>
                                                                ...
                                                                <span className='text-g d-block text-center font-weight-bold'>{item.attributes.userName}</span>
                                                            </div>
                                                                ...
                                                                <div className='d-flex align-items-center w-100'>
                                                                    ...
                                                                    <div className='ml-auto'>
                                                                        <Rating name="half-rating-read"
                                                                            value={parseFloat(item.attributes.rating)} precision={0.5} readOnly />
                                                                    </div>
                                                                </div>
                                            ...
                                            <form className='reviewForm' onSubmit={submitReview}>
                                                ...
                                                <div className='form-group'>
                                                    <textarea className='form-control' placeholder='Write a Review'
                                                        name="review" value={productReview} onChange={(e) => changeInput(e.target.name, e.target.value)}></textarea>
                                                </div>

                                                <div className='row'>
                                                    <div className='col-md-6'>
                                                        <div className='form-group'>
                                                            <input type='text' value={productReviewUserName} className='form-control' placeholder='Name' name='userName' onChange={(e) => changeInput(e.target.name, e.target.value)} />
                                                        </div>
                                                    </div>
