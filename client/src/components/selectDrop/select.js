    ...
    return (
        <ClickAwayListener onClickAway={()=>setisOpenSelect(false)}>
            <div className='selectDropWrapper cursor position-relative'>
                {icon}
                <span className='openSelect text-capitalize' onClick={openSelect}>{selectedItem.length>14 ? selectedItem.substr(0,14)+'...' :  selectedItem} 
                    ...
                    <div className='selectDrop'>
                        ...
                        <li key={0} onClick={() => closeSelect(0, placeholder)} className={`${selectedIndex === 0 ? 'active' : ''}`}>{placeholder}</li> 
                        {
                            ...
                            listData.map((item,index)=>{
                                ...
                                return(
                                    <li key={index+1} onClick={() => closeSelect(index+1, item)} className={`text-capitalize ${selectedIndex === index+1 ? 'active' : ''}`}>{item}</li> 
                                )
                            })
                        }
